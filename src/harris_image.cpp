...
Image make_1d_gaussian(float sigma)
{
  ...
  int kernel = (6*sigma) + 1;
  Image filter (kernel,1,1);
  float c = sqrtf(2 * M_PI) * sigma;

  for (int i = -filter.w/2; i < filter.w/2; i++) {

      float val = exp(-pow(i, 2.0)/(2.0 * pow(sigma, 2.0))) / c;
      filter(i+filter.w/2,0,0) = val;
  }
...
Image smooth_image(const Image &im, float sigma)
{
  ...
  // Image new_img = im.rgb_to_grayscale();
  // Image filter = make_gaussian_filter(sigma);
  // Image conv = convolve_image(new_img, filter, false);
...
Image structure_matrix(const Image &im2, float sigma)
{
  ...
  // Compute derivatives Ix Iy 
  Image fx = make_gx_filter();
  Image fy = make_gy_filter();

  Image Ix = convolve_image(im, fx, false);
  Image Iy = convolve_image(im, fy, false);

  // Compute derivatives multiplication IxIx IyIy and IxIy
  Image IxIx(Ix.w, Ix.h, Ix.c);
  Image IyIy(Ix.w, Ix.h, Ix.c);
  Image IxIy(Ix.w, Ix.h, Ix.c);
  for(int j=0; j<im.h; j++){
    for(int i=0; i<im.w; i++){
      IxIx(i,j) = Ix(i,j)*Ix(i,j);
      IyIy(i,j) = Iy(i,j)*Iy(i,j);
      IxIy(i,j) = Ix(i,j)*Iy(i,j);
    }
  }

  // Fill the structure matrix with the computed values
  for(int j=0; j<im.h; j++){
    for(int i=0; i<im.w; i++){
      S(i,j,0) = IxIx(i,j);
      S(i,j,1) = IyIy(i,j);
  ...
  // Apply gaussian filter to compute weighted sum
  Image filter = make_gaussian_filter(sigma);
  S = convolve_image(S, filter, true);
  return S;
}
...
Image cornerness_response(const Image &S, int method)
{
  ...
  // Find maximum and minimum deviation
  float trace;
  float det;
  float a, b, c;
  Image Str(2,2,1);

  // Compute structure matrix for each pixel
  for(int j=0; j<S.h; j++){
    for(int i=0; i<S.w; i++){
      a = S(i,j,0);
      b = S(i,j,1);
...
Image nms_image(const Image &im, int w)
{
  ...
  for(int j=0; j<im.h; j++){
    for(int i=0; i<im.w; i++){
      ...
      for(int v=(-w); v<w; v++){
        for(int u=(-w); u<w; u++){
          value = im.clamped_pixel(i+u, j+v);
          if(value > im(i,j)){
            r(i,j) = std::numeric_limits<T>::min();
          }
        }
      }
    }
...
vector<Descriptor> harris_corner_detector(const Image &im, float sigma, float thresh, int window, int nms, int corner_method)
{
  ...
  save_image(R, "output/R");

  // Run NMS on the responses
  Image Rnms = nms_image(R, nms);

  save_image(Rnms, "output/Rnms");
